...
# The class of the activity scheduler (used for lab 4)
class ActivityScheduler:
    # init function (constructor)
    def __init__(self, activities, prerequisites):
        self.activities = activities
        self.prerequisites = prerequisites
        self.act_graph = defaultdict(list)
    ...
    # function that adds an edge between vertices u and v to the graph
    def add_edge(self, u, v):
        self.act_graph[u].append(v)

    # function that helps us build the graph based on the activities and prerequisites
    def build_graph(self):
        for activity, prerequisite in zip(self.activities, self.prerequisites):
            self.add_edge(prerequisite, activity)

    # function that checks if the graph is a DAG graph
    def is_DAG(self):
        visited = set()
        stack = set()

        def dfs(dfs_node):
            visited.add(dfs_node)
            stack.add(dfs_node)

            for neighbor in self.act_graph[dfs_node]:
            ...
            stack.remove(dfs_node)
            return True

        for node in self.act_graph:
    ...
    # performs a topological sort (used for computing the earliest/latest times)
    def topological_sort(self):
        def dfs(dfs_node):
            nonlocal time

            self.start_time[dfs_node] = time
            time += 1

            for neighbor in self.act_graph[dfs_node]:
                if neighbor not in self.start_time:
                    dfs(neighbor)

            self.end_time[dfs_node] = time
            time += 1

        time = 1
        for node in self.act_graph:
            if node not in self.start_time:
                dfs(node)

    # function that computes the earliest and latest time
    def compute_earliest_latest_times(self):
        self.topological_sort()

        last_node = list(self.act_graph.keys())
        earliest_time = {node: 0 for node in self.act_graph}
        latest_time = {node: float('inf') for node in self.act_graph}
        latest_time[last_node] = earliest_time[last_node]

        for node in self.activities:
            for neighbor in self.act_graph[node]:
                earliest_time[neighbor] = max(earliest_time[neighbor], earliest_time[node] + self.activities[node])

        for node in reversed(list(self.act_graph.keys())):
            if node != last_node:
                for neighbor in self.act_graph[node]:
                    latest_time[node] = min(latest_time[node], latest_time[neighbor] - self.activities[node])

        self.total_time = earliest_time[last_node]
        self.critical_activities = [activity for activity in self.activities if
                                    earliest_time[activity] == latest_time[activity]]

        return earliest_time, latest_time

    # function that prints all the results in order to helps us see all the corresponding times.
    def print_results(self):
        earliest_time, latest_time = self.compute_earliest_latest_times()

        print("Topological sorting: ")
        for node in sorted(self.start_time, key=self.start_time.get):
            print(f"Activity: {node}, Earliest Start Time: {self.start_time[node]}, Latest Start Time: {self.end_time[node]}")

        print("\nEarliest and Latest Start Times: ")
        for activity in self.activities:
            print(f"Activity: {activity}, Earliest Start Time: {earliest_time[activity]}, Latest Start Time: {latest_time[activity]}")

        print("\nTotal Project Time: ", self.total_time)
        print("\nCritical Activities: ", self.critical_activities)
...
def print_options():
    ...
    print("LABORATORY 4")
    print("-----------------")
    print("41. Check if the list of activities is DAG and prints results.")
    print("0. Exit")
...
def menu():
    ...
    while True:
        ...
        if command == 1:
            ...
            print("How many activities do you want?")
            n = int(input(">"))

            if n <= 1:
                print("You need more activities.")
                return
            else:
                act = int(65)

                while n:
                    activities.append(chr(act))
                    act += 1
                    n -= 1
                ...
                for activity in activities:
                    if first:
                        ...
                        print("Prerequisite for ", activity, ": ")
                        prereq = input()
                        if prereq in activities:
                            prerequisites.append(prereq)
                        else:
                            prerequisites.append('A')

                scheduler = ActivityScheduler(activities, prerequisites)
